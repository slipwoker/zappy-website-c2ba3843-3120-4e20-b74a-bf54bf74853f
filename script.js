    ...
    "translations": {
      "he": {
        "consentModal": {
          ...
          "description": "עטוף באהבה משתמש בעוגיות כדי לשפר את החוויה שלך, לנתח שימוש באתר ולסייע במאמצי השיווק שלנו.",
    ...
    contactForm.addEventListener('submit', async function(e) {
        ...
        try {
            ...
            const response = await fetch('http://localhost:5001/api/email/contact-form', {
                ...
                body: JSON.stringify({
                    websiteId: 'c2ba3843-3120-4e20-b74a-bf54bf74853f',
                    name: data.name || '',
                    email: data.email || '',
...
async function fetchAdditionalJsSettings(force) {
  ...
  try {
    ...
    if (data.success && data.data) {
      ...
      if (data.data.productPageNote) {
        additionalJsProductPageNote = data.data.productPageNote;
      }
